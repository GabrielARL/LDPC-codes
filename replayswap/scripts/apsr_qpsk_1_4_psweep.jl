#!/usr/bin/env julia
# scripts/psr_qpsk_1_4_psweep.jl
#
# Pilot sweep (DFEC) — QPSK concat (256→512→1024), 3-way compare:
#   - TurboEQ (RSC concat dataset)
#   - EQ + SPA (LDPC concat dataset)   [now fixed to be true inner->outer SPA]
#   - JSDC (default = JT rounds) OR plain JSDC baseline
#
# Sweeps pilot ratio p (default: 0.0:0.1:0.5)
# Uses SAME selected blocks for all p (selected once by corr_thr + seed_sel)
#
# Writes:
#   (1) CSV with per-(p,blk) metrics:
#         data/runs/psr_qpsk_1_4_psweep.csv
#   (2) One constellation .jld2 per p in:
#         data/runs_psr_qpsk_1_4_constellations/
#       keys: s_jsdc_all, s_turbo_all, s_eq_all, meta(p=...)
#
# Run:
#   julia --project=. scripts/psr_qpsk_1_4_psweep.jl
#   julia --project=. scripts/psr_qpsk_1_4_psweep.jl --psweep "0.2,0.3,0.4,0.5" --corr 0.10 --nblk 225
#
# Notes (performance):
...
function main()
    ...
    outcsv   = joinpath(DATA_DIR, "runs", "psr_qpsk_1_4_psweep.csv")
    outdirC  = joinpath(DATA_DIR, "runs_psr_qpsk_1_4_constellations")
    ...
    println("==============================================================")
    @printf("PSR_QPSK_1_4 PSWEEP | corr_thr=%.2f | using=%d blocks | JSDC=%s\n",
    ...
    for (pi, p) in enumerate(psweep)
        ...
        for (ii, b) in enumerate(blk_list)
            ...
            # EQ+SPA (LDPC concat) — uses your FIXED decode_eq_spa in compare_3ways.jl
        ...
        # Save per-p constellation file (keys your grid plotter expects)
        meta = (p=p,
                corr_thr=corr_thr,
                seed_sel=seed_sel,
        ...
        outf = joinpath(outdirC, "psr_qpsk_1_4__$(ptag(p))__corr$(round(corr_thr,digits=2))__n$(length(blk_list)).jld2")
